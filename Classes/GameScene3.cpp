...
bool GameScene3::init() {
	...
	Bgm1->preloadBackgroundMusic("bgMusic4.wav");
	Bgm1->playBackgroundMusic("bgMusic4.wav", true);
...
void GameScene3::planeBomb(Vec2 vec, int tag)//爆炸效果
{
	float timeDelay = 0.1;
	...
	//参数中的回调函数会在动画播放完后调用
	Action * act = Sequence::create(Animate::create(ani), CCCallFuncN::create(blanksprite, callfuncN_selector(GameScene3::bombRemove)), NULL);
	this->addChild(blanksprite);
	blanksprite->setPosition(vec);
	blanksprite->runAction(act);
}
//移除动画
void GameScene3::bombRemove(Node * sprite)//删除自己
{
