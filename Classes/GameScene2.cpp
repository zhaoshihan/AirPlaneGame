...
bool GameScene2::init() {
	...
	//关卡2的背景音乐
	auto Bgm3 = SimpleAudioEngine::getInstance();
	Bgm3->preloadBackgroundMusic("bgMusicX.wav");
	Bgm3->playBackgroundMusic("bgMusicX.wav", true);
	Bgm3->setBackgroundMusicVolume(0.5);
	//²åÈë±³¾°
...
void GameScene2::planeBomb(Vec2 vec, int tag)//爆炸效果
{
	float timeDelay = 0.1;
	...
	//参数中的回调函数会在动画播放完后调用
	Action * act = Sequence::create(Animate::create(ani), CCCallFuncN::create(blanksprite, callfuncN_selector(GameScene2::bombRemove)), NULL);
	this->addChild(blanksprite);
	blanksprite->setPosition(vec);
	blanksprite->runAction(act);
}
//移除动画
void GameScene2::bombRemove(Node * sprite)//删除自己
{
